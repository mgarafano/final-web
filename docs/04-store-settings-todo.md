...
| # | Item | Where | What |
|---|---|---|---|
| 1 | Product photos are cropped in the grids | **Withdrawn** | v2 (published) used "Adapt to image", which made rows ragged; v3 tried a 4:5 box fitted, then filled; Raheem asked for the V1 photos back. v3 shows photos exactly as V1 did. |
...
- **Photos: back to V1.** Raheem, after two cuts of v3: "restore all photos to the
  state they were in in the V1". So the photo settings are exactly the first live
  theme's again — square box on the collection grid, homepage featured collection and
  related products, Dawn's stock search template, and no UMS photo rule at all. The
  4:5 box (fitted, then filled) and v2's "adapt" are gone. `templates/search.json` left
  the repo again.
